...
def cmd_process(args: argparse.Namespace) -> int:
    ...
    print(f"Processed {len(reviews)} question(s).")
    print(f"Reviews: {reviews_dir(output_root)}")
    print(f"High-yield list: {output_root / 'high-yield.html'}")
    if not args.review_only:
        print(f"Anki deck: {output_root / 'anki-bot.apkg'}")
    return 0
...
def cmd_build_apkg(args: argparse.Namespace) -> int:
    ...
    print(f"Rebuilt from {len(reviews)} review file(s), {count} note(s) in .apkg.")
    print(f"High-yield list: {output_root / 'high-yield.html'}")
    print(f"Anki deck: {output_root / 'anki-bot.apkg'}")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    process.add_argument(
        "--model",
        default=os.getenv("GEMINI_MODEL", "auto"),
        help="Gemini model: auto (default), gemini-2.5-flash, gemini-2.5-pro, or any model id",
    )
    process.add_argument(
        "--max-cards",
        type=int,
        default=int(os.getenv("ANKI_BOT_MAX_CARDS", "10")),
        help="Max cloze cards per question",
    )
    process.add_argument(
        "--deck",
    ...
    build.add_argument("--max-cards", type=int, default=10)
